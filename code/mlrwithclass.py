...
class Farm:
 ...
 #       return state           
    ...
    def rainfall(self,prevtemp,prevrainfall):
        ...
        for i in range (1,13):
            #print prev
            prevtemp=prevtemp+self.temp[index]
            self.temp_final.append(round((prevtemp/i),2))
            prevrainfall=prevrainfall+self.rainfall[index]
            self.rainfall_final.append(round(prevrainfall,2))
            index= index+1
            if index==12:
                index=0
    #print temp_avg
                ...
                for row in reader:
                   ...
                   if(narea>=ncrop and parea>=pcrop and karea>=kcrop):
                       ...
                       if no_months<13:        ###todo for duration>12months
                           ...
                           total=row[0]+","+str(self.rainfall_final[no_months-1])+","+str(self.temp_final[no_months-1])+","+ph+"\n"
    ...
    def regression(self):
        ...
        try:
           ...
           sorted_crops=self.quicksort(crop_name,crop_Y_pred,0,len(crop_Y_pred)-1)                       
           csvfile.close
           #return pred_crop
           return sorted_crops
    ...
    def quicksort(self,crop_name,crop_Y_pred,start, end):
        if start < end:
            # partition the list
            pivot = self.partition(crop_name,crop_Y_pred, start, end)
            # sort both halves
            self.quicksort(crop_name,crop_Y_pred, start, pivot-1)
            self.quicksort(crop_name,crop_Y_pred, pivot+1, end)
        return crop_name
    
    def partition(self,crop_name,crop_Y_pred, start, end):
    ...
    def ListtoStr(self,sorted_crop):
        pred_crop = ""
        comma_flag=0
...
#        return str(area)
    ...
    object=Farm()
   # area=object.mainarea()
    state=object.temperature(object.temp)
    #object.rain(object.rainfall)
    '''object.rainfall()
    object.nutrients()
    object.filewrite()
    sorted_crop=object.regression()
    final_crop=object.ListtoStr(sorted_crop)
    print (final_crop)
    '''
